...
df, survival_options, p_class_options, sex_options, embark_options, min_fare, max_fare, min_age, max_age
res=df

name_query=st.text_input("String match for name")

cols=st.columns(4)
survival=cols[0].miltiselect("Survived", survival_options)
p_class=cols[1].miltiselect("Passenger Class", p_class_options)
p_class=cols[2].miltiselect("Sex",sex_options)
embark=cols[3].multiseslect("Embarked", embark_options)

range_cols=st.columns(3)
min_fare_range, max_fare_range=range_cols[0].slider("Lowest Fare", float(min_fare), float(max_fare),
                                     [(flaot(min_fare), float(max_fare)]
min_fare_range, max_fare_range=range_cols[2].slider("Lowest Age", float(min_age), float(max_age),
...
removal_columns=st.multiselect("Select Columns to Remove", df.colums.tolist())
